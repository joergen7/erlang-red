...
handle_event(
    {'DOWN', _, process, _Pid, Reason},
    #{?GetWsName, '_sup_pid_' := SupPid} = NodeDef
) ->
    node_status(WsName, NodeDef, "dead", "blue", "ring"),
    ?SEND_STATUS(<<"dead">>),
    is_process_alive(SupPid) andalso exit(SupPid, Reason),
...
handle_event(
    ...
    #{?GetWsName, ?IsBeingSupervised} = NodeDef
) ->
    node_status(WsName, NodeDef, "dead", "blue", "ring"),
    ?SEND_STATUS(<<"dead">>),
    self() ! {stop, WsName},
...
handle_event({supervisor_started, SupMgrPid}, #{?GetWsName} = NodeDef) ->
...
%%
%%
handle_msg({incoming, #{?GetWsName} = Msg}, NodeDef) ->

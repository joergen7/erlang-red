...
debug_msg({ok,_WsName},_Type,_Data) ->
    ok;
debug_msg(_,_,_) ->
    ignore.
...
subscribe(WsName,NodeId,debug,Cb) ->
    gen_server:call(?MODULE, {subscribe_to_debug, WsName, NodeId, Cb});
...
handle_call({subscribe_to_status, WsName, NodeId, Cb}, _From, SubscriberStore) ->
    ...
    case maps:find(WsName,StatusStore) of
        {ok, Map} ->
            case maps:find(NodeId,Map) of
                {ok, Ary} ->
                    SStore2 = maps:put(WsName, maps:put(NodeId,[Cb|Ary],Map),
                                       StatusStore);
                _ ->
...
handle_call({subscribe_to_debug, WsName, NodeId, Cb}, _From, SubscriberStore) ->
    {ok, DebugStore} = maps:find(debug,SubscriberStore),
    ...
    case maps:find(WsName,DebugStore) of
        {ok, Map} ->
            case maps:find(NodeId,Map) of
                {ok, Ary} ->
                    DStore2 = maps:put(WsName, maps:put(NodeId,[Cb|Ary],Map),
                                       DebugStore);
                _ ->
                    DStore2 = maps:put(WsName, maps:put(NodeId,[Cb],Map),
                                       DebugStore)
            end;
        _ ->
            DStore2 = maps:put(WsName,maps:put(NodeId,[Cb],#{}),DebugStore)
    end,
